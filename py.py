...
async def on_member_join(member):
    ...
    channel = member.server.get_channel("585087748952817665")
    await app.send_message(channel, fmt.format(member, member.server))
    await app.send_message(member, "안녕? 난 뉴 배돌이야.")
    await app.send_message(member, "뭐 궁금한 점이 있으면 나한테 물어봐.")
    await app.send_message(member, "내가 알려줄 수 있는 범위 안에서 최선을 다해 알려줄테니까.")
    await app.send_message(member, "아 참! 우리 서버에 들어온 것을 환영해~")
