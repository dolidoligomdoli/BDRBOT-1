...
async def on_message(message):
    ...
    if message.content.startswith("!자기소개"):
        await message.channel.send("안녕? 난 뉴 배돌이야.")
        await message.channel.send("너희들이 디스코드 서버를 잘 이용할 수 있도록 ")
        await message.channel.send("도와주는 디스코드 PUBG MOBILE GLOBAL 서버 한정 어시스턴트야.")
        await message.channel.send("그런데 말이야...")
        await message.channel.send("난 나에 대해서 말하는 것을 별로 좋아하지 않아..")
        await message.channel.send("이정도면 됐지?")
